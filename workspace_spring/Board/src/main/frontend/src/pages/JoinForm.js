...
function JoinForm() {
  ...
  //회원가입 버튼 클릭 시 실행~
  function join(){
    //validation 처리 (유효성 검사 (숫자 특수문자 들어가는거 검사, 비번 맞게 입력 했는지 확인))
    ...
    <div className='join-div'>
      <div>
        ...
        <div className='id-area'>
          <input className='inp' name='memId' onChange={(e) => {changeJoinData(e)}} type='text'/>
          <button className='btn' onClick={(e) => {checkId()}}>중복확인</button>
        </div>
      </div>
      <div>
        <p>비밀번호</p>
        <input className='inp' type='password' name='memPw' onChange={(e) => {changeJoinData(e)}} />
      </div>
      <div>
      <p>비밀번호 확인</p>
      <input className='inp' type='password' name='confirmPw' onChange={(e) => {changeJoinData(e)}}/>
      </div>
      <div>
      <p>이름</p>
      <input className='inp' type='text' name='memName' onChange={(e) => {changeJoinData(e)}} />
      </div>
      <div className='gender-div'>
        <div><span>성별</span></div>
        <div>
          <input type='radio' name='gender' value="male" onChange={(e) => {changeJoinData(e)}} 
          checked={joinData.gender == 'male'} 
          />남
        </div>

        <div>
          <input type='radio' name='gender' value="female" onChange={(e) => {changeJoinData(e)}}
