...
useEffect(() => {
  ...
  function insertItem(){
    ...
    //위의 설정코드를 axios 통신할 떄 추가하면 자바로 넘어가는 데이터를 전달하는 방식이 달라짐
  ...
  return (
    <div className='reg-item-div'>
      ...
      <div className='input-element'>
        <div>상품명</div>
        <div><input type='text' className='form-control' name='itemName' onChange={(e) => {changeInsertItemData(e)}}/></div>
      </div>
      <div className='input-element'>
        <div>상품 가격</div>
        <div><input type='text' className='form-control' name='itemPrice' onChange={(e) => {changeInsertItemData(e)}}/></div>
      </div>
      <div className='input-element'>
        <div>상품 소개</div>
        <div >
          <textarea className='form-control' rows={7} name='itemIntro' onChange={(e) => {changeInsertItemData(e)}}></textarea>
        </div>
      </div>
      <div className='input-element'>
