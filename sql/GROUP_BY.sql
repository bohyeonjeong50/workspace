...
-- 사원 중에서 가장 많은 급여를 조회
SELECT MAX(SAL), EMPNO FROM emp;
SELECT CONCAT(SAL, EMPNO) FROM emp;

-- 각 부서별로 가장 많은 급여를 조회
-- 단일행 함수와 조회 결과 가지는 행의 갯수가 다른 컬럼을 사용 못하지만
-- GROUP BY 절에서는 유일하게 그루핑한 컬럼은 조회가 가능 함.
SELECT DEPTNO,  MAX(SAL), SUM(SAL), COUNT(SAL), AVG(SAL)
FROM emp
GROUP BY DEPTNO;

-- 직급이 사원이 아닌 직급들 중 직급별 급여의 평균을 조회
...
-- 조회 결과 데이터는 급여의 부서별 급여의 합 기준 오름차순으로 정렬
