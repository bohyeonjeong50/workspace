...
-- 기본키(PRIMARY KEY) :하나의 행을 식별할 수 있는 하나 이상의 컬럼
-- 후보키(CANDUDATE KEY) : 기본 키가 될 수 있는 후보
-- 기본키(PRIMARY KEY, PK) : 후보키 중에서 각 행을 구분짓기 위해 결정한 키
...
CREATE TABLE BOARD (
	...
	, CONTENT VARCHAR(100)
	, MEM_ID VARCHAR(50) REFERENCES BOARD_MEMBER (MEM_ID) -- me> 외래키(REFERENCES) : 어떤 테이블에서 참조했는지
...
-- ORDER BY CREATE_DATE DESC
ORDER BY BOARD_NUM DESC, REPLY_NUM DESC; -- DESC 내림차순
